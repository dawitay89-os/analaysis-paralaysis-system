  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center gap-3 text-indigo-400 mb-6">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-theme-text">{t('weekly_extraction')}</h1>
      </div>

      <div className="flex gap-2 mb-8">
        {SECTIONS.map((_, i) => (
          <div key={i} className={`h-1.5 flex-1 rounded-full ${i <= currentSection ? 'bg-indigo-500' : 'bg-theme-active'}`} />
        ))}
      </div>

      <div className="bg-theme-card border border-theme-border p-6 md:p-8 rounded-xl shadow-xl space-y-8">
        <div>
          <h2 className="text-xl font-bold text-theme-text">{section.title}</h2>
          ...
          {section.questions.map((q, i) => (
            <div key={i} className="space-y-4">
              <div className="flex justify-between items-end">
                <label className="text-sm font-medium text-theme-text w-3/4">{q}</label>
              ...
              <input
                ...
                className="w-full h-2 bg-theme-active rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-400 transition-all"
              />
              <div className="flex justify-between text-xs text-theme-muted font-mono">
                <span>1</span>
                <span>5</span>
                <span>10</span>
              </div>
