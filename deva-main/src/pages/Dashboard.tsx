...
export function Dashboard() {
  ...
  if (!state.profile) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        <p className="text-zinc-400">Please complete your profile to view the dashboard.</p>
      </div>
    );
  }
  ...
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        <p className="text-zinc-400">No assessments taken yet. Take your first weekly assessment.</p>
      </div>
    );
  }
  ...
  const radarData = {
    labels: ['Paralysis', 'FOMO', 'Fear', 'Hesitation', 'Addiction'],
    datasets: [
      {
        label: 'Psychological Load',
        data: [
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Hello, {state.profile.name || 'Trader'}. Here is your psychological footprint.</p>
        </div>
        <div className="bg-indigo-600/10 border border-indigo-500/20 px-4 py-2 rounded-lg">
          <span className="text-xs text-indigo-400 uppercase font-bold tracking-wider block">Identified Archetype</span>
          <span className="text-lg font-bold text-indigo-100">{latest.archetype}</span>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <ScoreCard title="Discipline" value={latest.scores.discipline} color="text-emerald-400" />
        <ScoreCard title="Confidence" value={latest.scores.confidence} color="text-blue-400" />
        <ScoreCard title="Paralysis" value={latest.scores.analysisParalysis} color="text-red-400" />
        <ScoreCard title="FOMO" value={latest.scores.fomo} color="text-amber-400" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#121214] p-6 rounded-xl border border-zinc-800 md:col-span-2 space-y-4">
          <h3 className="font-bold">Historical Performance</h3>
        ...
        <div className="bg-[#121214] p-6 rounded-xl border border-zinc-800 space-y-4">
          <h3 className="font-bold">Psychological Load</h3>
          <div className="h-[300px] w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-[#121214] border border-zinc-800 p-6 rounded-xl space-y-6">
          <h3 className="font-bold border-b border-zinc-800 pb-2">Active Root Causes</h3>
            ...
            {latest.rootCauses.map((cause, i) => (
              <div key={i} className="flex justify-between items-center bg-zinc-900/50 p-3 rounded-lg border border-zinc-800/50">
                <div>
                  ...
                  <span className="text-xs text-zinc-500 uppercase">{cause.type} factor</span>
                </div>
        ...
        <div className="bg-[#121214] border border-zinc-800 p-6 rounded-xl space-y-6">
          <h3 className="font-bold border-b border-zinc-800 pb-2">Prescribed Coaching Actions</h3>
          <div className="space-y-4">
